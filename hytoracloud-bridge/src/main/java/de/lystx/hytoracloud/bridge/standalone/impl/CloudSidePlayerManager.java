...
@Setter @Getter
public class CloudSidePlayerManager implements ICloudPlayerManager {
    ...
    public CloudPlayer getPlayer(String name) {
        return CloudDriver.getInstance().getResponse(new ResultPacketCloudPlayer(name)).get(0).asCustom(CloudPlayer.TYPE_CLASS);
    }

    public CloudPlayer getPlayer(UUID uniqueId) {
        return CloudDriver.getInstance().getResponse(new ResultPacketCloudPlayer(uniqueId)).get(0).asCustom(CloudPlayer.TYPE_CLASS);
    }
    ...
    public void getAsync(String name, Consumer<CloudPlayer> consumer) {
        try {
            ...
            consumer.accept(response.get(0).asCustom(CloudPlayer.TYPE_CLASS));
