...
import de.lystx.cloudsystem.library.network.packet.response.ResponseScope;
import de.lystx.cloudsystem.library.network.extra.util.LazySupplier;
import de.lystx.cloudsystem.library.network.extra.util.NettyUtil;
...
public final class PacketMessenger {
    ...
    public final synchronized <R> R send(AbstractPacket packet, Consumer<R>... consumers) {
        ...
        if (protocolId == -1) {
            throw new Exception("Can't send Packet because it's not registered! (id: " + protocolId + ")");
        }
        ...
        if(target.size() == 0) {
            Cloud.getInstance().getEventService().callEvent(new PacketQueueEvent(null, packet, packetConsumer));
        }
        else {
            for(Channel channel : target) {
                Cloud.getInstance().getEventService().callEvent(new PacketQueueEvent(channel, packet, packetConsumer));
            }
        }
