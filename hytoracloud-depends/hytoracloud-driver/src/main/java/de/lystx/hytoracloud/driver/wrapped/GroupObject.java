...
public class GroupObject extends WrappedObject<IServiceGroup, GroupObject> implements IServiceGroup {
    ...
    public void update() {
        if (CloudDriver.getInstance().getDriverType() == CloudType.CLOUDSYSTEM) {
            CloudDriver.getInstance().getGroupManager().update(this);
            return;
        }
    ...
    public int getMaxPlayers() {
        if (this.environment == ServerEnvironment.PROXY) {
            int proxies = getServices().size();
            int maxPlayers = CloudDriver.getInstance().getConfigManager().getNetworkConfig().getMaxPlayers();
    ...
    @Override
    public ServiceBuilder prepareService() {
    ...
    public IServiceGroup sync() {
        return CloudDriver.getInstance().getGroupManager() == null ? null : CloudDriver.getInstance().getGroupManager().getCachedObject(this.name);
    }
    ...
    public List<ICloudPlayer> getPlayers() {
        ...
        if (this.environment == ServerEnvironment.SPIGOT) {
    ...
    @Override
    public Class<GroupObject> getWrapperClass() {
