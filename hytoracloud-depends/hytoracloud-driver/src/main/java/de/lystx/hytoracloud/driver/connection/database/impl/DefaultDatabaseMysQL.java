...
public class DefaultDatabaseMysQL implements IDatabase {
    ...
    public void connect() {
        ...
        if (!mySQLConnection.connect()) {
            CloudDriver.getInstance().getParent().getConsole().sendMessage("ERROR", "§cCouldn't connect to §eMySQL Database§c!");
        } else {
            ...
            CloudDriver.getInstance().getParent().getConsole().sendMessage("INFO", "§9Connected to §bMySQL Database§9!");
        }
    }
