...
public class PlayerObject extends WrappedObject<ICloudPlayer, PlayerObject> implements ICloudPlayer {
    ...
    public IPlayerSettings getSettings() {
        if (CloudDriver.getInstance().getDriverType() == CloudType.BRIDGE && CloudDriver.getInstance().getServiceType().isProxy()) {
            return DriverQuery.dummy("PLAYER_GET_SETTINGS", CloudDriver.getInstance().getProxyBridge().getSettings(this.getUniqueId())).pullValue();
        }
    ...
    public void sendMessage(ChatComponent chatComponent) {
        if (CloudDriver.getInstance().getDriverType() == CloudType.BRIDGE) {
            if (CloudDriver.getInstance().getServiceManager().getThisService().getGroup().getType() == ServiceType.PROXY) {
