package de.lystx.hytoracloud.driver.connection.protocol.hytora.connection.server;
...
import de.lystx.hytoracloud.driver.connection.protocol.hytora.elements.component.RepliableComponent;
import de.lystx.hytoracloud.driver.connection.protocol.hytora.elements.other.UserManager;
import de.lystx.hytoracloud.driver.connection.protocol.hytora.elements.packet.PacketManager;
...
@Getter
public class NetworkServer implements INetworkConnection {
     ...
     */
    @Override
    public Future<INetworkConnection> createConnection() {

        CompletableFuture<INetworkConnection> future = new CompletableFuture<>();
        ...
        service.execute(() -> {
            try (ServerSocket server = new ServerSocket(port)) {
                ...
                while (this.opened && !server.isClosed()) {
                    ...
                    NetworkBridge bridge = new NetworkBridge(this, socket);
                }
            } catch (Exception e) {
    ...
    public void sendObject(Serializable object, String receiver) {
        try {
            if (receiver.equalsIgnoreCase("ALL")) {
                for (NetworkBridge user : this.userManager.getConnectedUsers()) {
                ...
                for (NetworkBridge user : this.userManager.getUsers(receiver)) {
                    synchronized (user.getSyncOut()) {
                        user.getObjectOutputStream().writeObject(object);
                        user.getObjectOutputStream().flush();
                    }
     ...
     * @param consumer the consumer
     */
    public void registerLoginHandler(Consumer<NetworkBridge> consumer) {
