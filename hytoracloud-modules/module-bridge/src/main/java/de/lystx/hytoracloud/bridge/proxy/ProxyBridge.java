...
public interface ProxyBridge {
    ...
    default EventResult playerLogin(IPlayerConnection connection) {
        ...
        if (cachedPlayer != null) {
            ...
            if (CloudDriver.getInstance().getFallbackManager().getFallback(cachedPlayer) != null) {
                ...
                if (!CloudDriver.getInstance().getEventManager().callEvent(playerJoin)) {
                    if (CloudDriver.getInstance().getConfigManager().getProxyConfig().isEnabled()) {

                        if (CloudDriver.getInstance().getConfigManager().getNetworkConfig().isMaintenance()
                                && !CloudDriver.getInstance().getConfigManager().getNetworkConfig().getWhitelistedPlayers().contains(cachedPlayer.getName())
                                && !CloudDriver.getInstance().getPermissionPool().hasPermission(cachedPlayer.getUniqueId(), "cloudsystem.network.maintenance")) {

                            event.setCancelled(true);
                            event.setComponent(CloudDriver.getInstance().getConfigManager().getNetworkConfig().getMessageConfig().getMaintenanceNetwork().replace("&", "§").replace("%prefix%", CloudDriver.getInstance().getPrefix()));
                        }

                        if ((CloudDriver.getInstance().getPlayerManager().getCachedObjects().size() + 1) >= CloudDriver.getInstance().getConfigManager().getNetworkConfig().getMaxPlayers()) {
                            event.setCancelled(true);
                ...
                event.setCancelled(true);
                event.setComponent(CloudDriver.getInstance().getConfigManager().getNetworkConfig().getMessageConfig().getNoLobbyFound().replace("%prefix%", CloudDriver.getInstance().getPrefix()));
            }
        }
    ...
    default void playerQuit(ICloudPlayer player) {
        ...
        CloudDriver.getInstance().getEventManager().callEvent(playerQuit);
   ...
   default boolean commandExecute(ICloudPlayer player, String rawLine) {
       ...
       if (CloudDriver.getInstance().getCommandManager().getCommand(command) != null) {
           CloudDriver.getInstance().getCommandManager().executeCommand(player, true, rawLine);
           return true;
       }
       return false;
   }
    ...
    default void onServerConnect(ICloudPlayer cloudPlayer, IService service) {
        ...
        playerObject.update();
        CloudDriver.getInstance().getEventManager().callEvent(new DriverEventPlayerServerChange(playerObject, service));
    }
