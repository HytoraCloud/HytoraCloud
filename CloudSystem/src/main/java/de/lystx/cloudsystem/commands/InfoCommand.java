...
public class InfoCommand extends CloudCommand implements TabCompletable {
    ...
    public void execute(CloudLibrary cloudLibrary, CloudConsole console, String command, String[] args) {
        if (args.length == 1) {
            ...
            console.getLogger().sendMessage("INFO", "§bTPS §a: §f" + networkInfo.formatTps(CloudSystem.getInstance().getTicksPerSecond().getTPS()));
            console.getLogger().sendMessage("INFO", "§bCloud-Host §a: §f" + CloudSystem.getInstance().getService(ConfigService.class).getNetworkConfig().getHost());
            console.getLogger().sendMessage("INFO", "§bCloud-Port §a: §f" + CloudSystem.getInstance().getService(ConfigService.class).getNetworkConfig().getPort());
            console.getLogger().sendMessage("INFO", "§7----------------------------------");
