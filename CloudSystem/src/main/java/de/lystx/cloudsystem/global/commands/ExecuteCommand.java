...
public class ExecuteCommand extends CloudCommand implements TabCompletable {
    ...
    public void execute(CloudLibrary cloudLibrary, CloudConsole console, String command, String[] args) {
        if (args.length > 1) {
            ...
            Service service = cloudLibrary.getService(ServerService.class).getService(server);
            if (service == null) {
            ...
            cloudLibrary.getService(CloudNetworkService.class).sendPacket(new PacketPlayOutExecuteCommand(service.getName(), sb.toString()));
