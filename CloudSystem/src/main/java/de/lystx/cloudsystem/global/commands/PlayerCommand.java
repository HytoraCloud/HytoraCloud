...
public class PlayerCommand extends CloudCommand {
    ...
    public void execute(CloudLibrary cloudLibrary, CloudConsole console, String command, String[] args) {
        ...
        if (args.length == 1) {
            ...
            cloudPlayer.kick(cloudLibrary.getService(CloudNetworkService.class).getCloudServer(), sb.toString());
            console.getLogger().sendMessage("INFO", "§7The player §b" + cloudPlayer.getName() + " §7was kicked for §a" + sb.toString() + "§7!");
        } else {
            this.correctSyntax(console);
        }
    }
    ...
    public void correctSyntax(CloudConsole console) {
        ...
        console.getLogger().sendMessage("INFO", "§9players <kick> <player> <reason> §7| Kicks a player ");
    }
}
