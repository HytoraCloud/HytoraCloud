        ...
        if (this.packetPlayOutCustomPayloadClass != null) {
            ...
            if (!this.customPayloadHasBytes) {
                try {
                    ...
                    CloudServer.getInstance().getLogger().severe("Couldn't find a valid constructor for PacketPlayOutCustomPayload. Disabling the plugin.");
                    Bukkit.getPluginManager().disablePlugin(CloudServer.getInstance());
                }
            }
        }
   ...
   public void sendPacket(Player player, Object packet) {
    ...
    public Object getPluginMessagePacket(String channel, byte[] bytes) {
        try {
            ...
            CloudServer.getInstance().getLogger().severe("Couldn't construct a custom-payload packet (Channel: " + channel + "):");
