...
public class ServerCatcher {
     ...
     */
    public void registerChannelHandler(String name, Consumer<ReplyComponent> consumer) {
        Set<Consumer<ReplyComponent>> candidate = channelHandlers.get(name);
        if (candidate != null) {
    ...
    public void handleComponent(Component hytoraComponent, HytoraConnectionBridge client) {
        ...
        if (candidates != null) {
            for (Consumer<ReplyComponent> consumer : candidates) {
                consumer.accept(new ServerComponent(client, hytoraComponent));
            }
        }
    }
