...
public class PermissionPool implements Serializable, ObjectPool<OfflinePlayer> {
    ...
    public void removePermissionGroupFromUser(UUID uniqueId, PermissionGroup group) {
        ...
        CloudDriver.getInstance().getEventManager().callEvent(new DriverEventPlayerGroupRemove(this.getNameByUUID(uniqueId), group));
    }
    ...
    public void addPermissionGroupToUser(UUID uniqueId, PermissionGroup group, Integer time, PermissionValidity validity) {
        ...
        this.update(offlinePlayer);
        CloudDriver.getInstance().getEventManager().callEvent(new DriverEventPlayerGroupReceive(getNameByUUID(uniqueId), group, time, validity));
    }
     ...
     */
    public void update() {
        CloudDriver.getInstance().setInstance("permissionPool", this);
        if (CloudDriver.getInstance().getDriverType().equals(CloudType.CLOUDSYSTEM)) {
            PermissionService instance = CloudDriver.getInstance().getServiceRegistry().getInstance(PermissionService.class);
            if (instance != null) {
                instance.setIgnore(true);
                instance.save(instance.getFile(), CloudDriver.getInstance().getServiceRegistry().getInstance(FileService.class).getCloudPlayerDirectory(), CloudDriver.getInstance().getDatabaseManager().getDatabase());
                instance.reload();
            }
        }
