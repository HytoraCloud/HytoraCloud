package de.lystx.hytoracloud.driver.setup;
...
public abstract class SetupExecutor<T> {
    ...
    public void start(Consumer<T> consumer) {
        ...
        this.console.sendMessage("§9");
        this.console.sendMessage("§9");

        //Sending info that its cancellable
        if (this.isCancellable()) {
            this.console.sendMessage("SETUP", "§aIf you want to abort this setup just type §2'cancel'§a!");
        }
        ...
        this.console.getConsoleReader().setPrompt("");
        this.console.sendMessage("SETUP", this.setup.getValue().question());
        ...
        while (this.current < this.map.size() + 1) {
            try {
                ...
                String line = this.console.getConsoleReader().readLine(ConsoleColor.formatColorString(this.console.getPrefix()));
                if (line != null) {
                    this.console.getConsoleReader().setPrompt("");
    ...
    public void handleQuestion(String input) {
        if (this.setup != null) {
            ...
            if (input.trim().isEmpty()) {
                this.console.sendMessage("ERROR", "§cPlease do not enter §eempty §cinput!");
                return;
            }

            //Cancelling setup
            if (input.equalsIgnoreCase("cancel")) {
                this.console.sendMessage("SETUP", "§cThe current setup was §ecancelled§c!");
                this.cancelled = true;
                this.current += 10000;
            ...
            if (!isAnswerEnumOnly(this.setup.getValue(), input)) {
                this.console.sendMessage("ERROR", "§cPossible answers: §e"  + Arrays.toString(setup.getValue().enumOnly().getEnumConstants()).replace("]", ")").replace("[", "("));
                return;
            }
            ...
            if (!isAnswerAllowed(this.setup.getValue(), input)) {
                this.console.sendMessage("ERROR", "§cPossible answers: §e"  + Arrays.toString(this.setup.getValue().onlyAnswers()).replace("]", "").replace("[", ""));
                this.console.sendMessage("ERROR", "§cRequired Type: §e" + this.setup.getKey().getType().getSimpleName());
                return;
            }

            //If the current input is forbidden to use
            if (this.isAnswerForbidden(this.setup.getValue(), input)) {
                this.console.sendMessage("ERROR", !input.trim().isEmpty() ? ("§cThe answer '§e" + input + " §cmay not be used for this question!") : "§cThis §eanswer §cmay not be used for this question!");
                return;
            }
            ...
            try {
                ...
                if (value == null) {
                    this.console.sendMessage("ERROR", "§cPlease try again");
                    return;
                }
                ...
                this.console.sendMessage("ERROR", "§cThe §einput §cdidn't match any of the available §eAnswerTypes§c!");
                return;
            }
        ...
        if (this.setup != null) {
            ...
            this.console.getConsoleReader().setPrompt("");
            this.console.sendMessage("SETUP", "§b" + this.setup.getValue().question());
            this.printExtraMessage();
            this.console.getConsoleReader().setPrompt("");
        } else {
            this.exit();
    ...
    public void printExtraMessage() {
        ...
        for (String s : this.setup.getValue().message()) {
            try {
                ...
                this.console.sendMessage(prefix, message);
            } catch (Exception e) {
                this.console.sendMessage(s);
            }
        }
    }
    ...
    private void printHeader() {
        ...
        this.console.sendMessage("§8");
        this.console.sendMessage(this.htmlHeader);
        this.console.sendMessage("§8");
        this.console.sendMessage("INFO", "§7» §7Setup §f: §b" + getClass().getSimpleName());
        this.console.sendMessage("INFO", "§7» §7Cancellable §f: §b" + (this.cancellable ? "Yes" : "No"));
        this.console.sendMessage("INFO", "§7» §7Question §f: §b" + (this.current == 1 ? 1 : current ) + "/" + (this.map.keySet().size() == 0 ? "Loading" : this.map.keySet().size() + ""));
        this.console.sendMessage("§8");
    }
