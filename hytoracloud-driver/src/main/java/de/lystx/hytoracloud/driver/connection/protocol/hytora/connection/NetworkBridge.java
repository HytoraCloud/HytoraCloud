...
@Setter @Getter
public class NetworkBridge implements IProtocolSender {
     ...
     */
    public NetworkBridge(NetworkServer server, Socket socket) {
        this.server = server;
        this.socket = socket;
            ...
            try (
                ...
                if (this.waitLogin()) {
                    ...
                    if (!this.server.getAcceptedNames().contains(this.name)) {
                        ...
                        for (Consumer<NetworkBridge> connectionHandler : this.server.getCatcher().getConnectionHandlers()) {
    ...
    /**
     * Processes a {@link Packet} into the bridge
     *
