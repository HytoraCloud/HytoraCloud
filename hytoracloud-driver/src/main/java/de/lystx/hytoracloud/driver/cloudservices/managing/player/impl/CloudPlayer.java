...
public class CloudPlayer implements Serializable, CloudCommandSender , IPermissionUser, Identifiable {
    ...
    public void setService(IService IService) {
        this.service = IService == null ? "No Server" : IService.getName();
    }
     ...
     *
     * @param IService the proxy
     */
    public void setProxy(IService IService) {
        this.proxy = IService.getName();
    }
    ...
    public void playSound(Enum<?> sound, Float v1, Float v2) {
        if (CloudDriver.getInstance().getDriverType().equals(CloudType.BRIDGE) && CloudDriver.getInstance().getThisService().getGroup().getType() == ServiceType.SPIGOT) {
     ...
     */
    public void connectRandom(IServiceGroup IServiceGroup) {

        List<IService> IServices = CloudDriver.getInstance().getServiceManager().getServices(IServiceGroup);
