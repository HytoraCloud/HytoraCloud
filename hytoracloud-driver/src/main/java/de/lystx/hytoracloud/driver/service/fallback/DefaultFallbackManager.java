...
public class DefaultFallbackManager implements IFallbackManager {
    ...
    public IService getFallback(ICloudPlayer player) {
        try {
            ...
            IServiceGroup serviceGroup = CloudDriver.getInstance().getGroupManager().getCachedObject(fallback.getGroupName());
            IService service;
            try {
    ...
    public IService getFallbackExcept(ICloudPlayer player, IService service) {
        Fallback fallback = this.getHighestFallback(player);
        List<IService> services = CloudDriver.getInstance().getServiceManager().getCachedObjects(CloudDriver.getInstance().getGroupManager().getCachedObject(fallback.getGroupName()));
    ...
    public Fallback getHighestFallback(ICloudPlayer player) {
        ...
        return list.get(list.size() - 1) == null ? CloudDriver.getInstance().getConfigManager().getNetworkConfig().getFallbackConfig().getDefaultFallback() : list.get(list.size() - 1);
    }

    @Override
    public List<Fallback> getFallbacks(ICloudPlayer player) {
        List<Fallback> list = new LinkedList<>();
        list.add(CloudDriver.getInstance().getConfigManager().getNetworkConfig().getFallbackConfig().getDefaultFallback());
